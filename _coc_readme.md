...
- `web/` ΓÇö Next.js app (Clerk auth + UI)
- `api/` ΓÇö FastAPI service (resume analysis, matching, interviews, voice coach)
- `db/` ΓÇö Postgres schema + job seed
...
1. Go to [https://supabase.com/dashboard](https://supabase.com/dashboard) ΓåÆ **New project**.
2. Wait until the project is **Healthy**.
3. Open **Project Settings ΓåÆ Database**.

### 2. Connection string for FastAPI (`api/.env`)

Use the **direct** database URL for a longΓÇærunning Python server (not serverless):
...
# Preferred: OpenAI ΓÇö https://platform.openai.com/api-keys (starts with sk-ΓÇª)
OPENAI_API_KEY=sk-...
# Optional: OPENAI_MODEL=gpt-4o-mini  OPENAI_TRANSCRIBE_MODEL=whisper-1  OPENAI_BASE_URL=https://api.openai.com/v1

# Only used when OPENAI_API_KEY is unset ΓÇö Google AI Studio https://aistudio.google.com/apikey (AIzaSyΓÇª)
GEMINI_API_KEY=
...
- If the dashboard URI uses **`postgres://`**, that is fine ΓÇö the API normalizes it.
- URLΓÇæencode special characters in the password (e.g. `@` ΓåÆ `%40`).
...
Supabase does **not** autoΓÇærun our repo SQL.

1. In Supabase: **SQL Editor ΓåÆ New query**.
2. Paste the full contents of [`db/schema.sql`](db/schema.sql) ΓåÆ **Run**.
...
- In the app: **Internships ΓåÆ Seed jobs**, or  
...
# Agentic Premier League 2026 ≡ƒÜÇ
...
Γ£à Be respectful and professional
Γ£à Collaborate positively with teammates and mentors
Γ£à Encourage learning and knowledge sharing
Γ£à Provide constructive feedback
Γ£à Respect diverse opinions and backgrounds
Γ£à Follow repository contribution workflows
Γ£à Maintain professionalism during demos, discussions, and reviews
Γ£à Build responsibly using ethical AI practices
Γ£à Keep repositories clean, organized, and properly documented
...
Keys: Clerk Dashboard ΓåÆ your application ΓåÆ **API Keys**.
...
3. Run `db/schema.sql` with `psql`, or paste it into SupabaseΓÇæequivalent tooling.
...
2. **LLM**: Prefer **`OPENAI_API_KEY`** in `api/.env` ([OpenAI API keys](https://platform.openai.com/api-keys), **`sk-ΓÇª`**). If unset, the API falls back to **`GEMINI_API_KEY`** from [Google AI Studio](https://aistudio.google.com/apikey) (**`AIzaSyΓÇª`**).
...
### ΓÇ£Failed to fetchΓÇ¥ on Resume / Internships

- Run **FastAPI** on port **8000**: `uvicorn app.main:app --reload --port 8000` from `api/`.
- Default setup uses **`NEXT_PUBLIC_API_BASE_URL=/api-backend`** ΓÇö restart **`npm run dev`** after changing `web/.env.local` or `next.config.ts`.
- Behind the proxy, **`BACKEND_URL`** (default `http://127.0.0.1:8000`) must match where uvicorn listens ΓÇö Next forwards `/api-backend/*` there.
...
1. Resume: upload PDF ΓåÆ ATS + skills + fixes  
2. Internships: **Seed jobs** ΓåÆ **Get matches**  
3. Voice: record ΓåÆ transcript + filler metrics + coaching  
=======
...
Γ£à Ensure the project runs correctly
Γ£à Update documentation if required
Γ£à Remove unnecessary files and logs
Γ£à Verify no secrets/API keys are committed
Γ£à Ensure commit history is clean and meaningful
...
Γ¥î `.env` files
Γ¥î API keys
Γ¥î Database credentials
Γ¥î Access tokens
Γ¥î Private certificates

Instead:

Γ£à Use `.env.example` files
Γ£à Store secrets securely
Γ£à Rotate exposed keys immediately if leaked
...
Γ¥î Harassment or discrimination
Γ¥î Hate speech or offensive content
Γ¥î Bullying or personal attacks
Γ¥î Plagiarism without attribution
Γ¥î Spamming repositories or discussions
Γ¥î Uploading malicious or harmful code
Γ¥î Intentionally sabotaging another teamΓÇÖs work
Γ¥î Unauthorized access attempts
Γ¥î Creating unethical or unsafe AI systems
Γ¥î Sharing illegal, harmful, or abusive content
...
Γ£à Learn from tutorials, frameworks, and public resources
Γ£à Use open-source tools with attribution
Γ£à Discuss ideas with mentors and other participants

Teams may NOT:

Γ¥î Copy full projects without meaningful contribution
Γ¥î Re-upload another teamΓÇÖs work
Γ¥î Misrepresent AI-generated work as fully original
...
Γ¥î Do not upload `node_modules/`
Γ¥î Do not upload large datasets or unnecessary binaries
Γ¥î Do not commit build folders unless required
Γ¥î Do not spam commits with meaningless messages

Recommended:

Γ£à Use `.gitignore` properly
Γ£à Keep repositories lightweight and organized
Γ£à Upload large videos/assets externally if needed
...
LetΓÇÖs build the future of Agentic AI together ≡ƒÜÇ

---

### Organized with Γ¥ñ∩╕Å by GDG Nagpur
>>>>>>> f9c6337ba58985138834d0eb19f66f52c224b2e1
